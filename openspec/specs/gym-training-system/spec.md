...
Define the offline-first Gym V2 training contract for stable exercise identity,
typed routines, weekly planning, modality-aware sessions, progression, body
weight, progress analytics, and ownership-safe recovery.
...
effects. Current Scope-7 manifests/files SHALL be recoverable, and frozen
Scope-6 manifests/files SHALL remain restorable.

#### Scenario: Current scope round trip preserves training meaning
...
#### Scenario: Frozen scope-6 backup remains compatible

- **GIVEN** a valid historical Scope-6 manifest or portable file
- **WHEN** it is validated and restored through the compatibility path
- **THEN** it is accepted without requiring Scope-7 fields
