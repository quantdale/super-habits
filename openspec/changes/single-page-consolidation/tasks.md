...
- [x] **3.7** Smoke test on at least one native target to confirm gesture dead zones and modal behavior. **DONE 2026-08-05 on a local headless Android emulator (AVD `superhabits`, android-35 x86_64, WHPX-accelerated, debug APK via `expo prebuild` + Gradle). Verified on native: (1) all six sections switch with correct content; (2) the 40px left/right edge dead zones on the section-switch pan — swipes originating at the edge moving toward the edge do NOT switch, normal swipes do (RNGH `activeOffsetX` + `absoluteX < 40` semantics confirmed); (3) the Settings modal opens and closes via Back; (4) the Command Center overlay opens and closes, with the Ask/Create/Auto mode toggle rendering (AI Ask feature live on native). One sub-check not completed on native: the Pomodoro timer running across section switches — the emulator repeatedly crashed/hung under machine memory exhaustion (host had ~1–2GB free of 32GB due to WSL/browsers; 6 boot attempts, the final minimal-memory config booted but died mid-test). That flow is covered by web E2E journey J1 (timer survives every section switch, row-oracle asserted, 8/8 green) and the native Focus section + section switching were each individually verified. Recommended follow-up: re-run the timer check on a machine with ≥8GB free memory (`emulator -avd superhabits -no-window -gpu swiftshader_indirect -memory 2048`, then `npm start` + launch the installed debug app).**
