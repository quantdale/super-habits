...
Propagating a defaults change SHALL update only the *default* durations used for a fresh idle timer. An in-flight session's retained state — remaining time and running/paused status — SHALL be left untouched, and no session SHALL be logged as a side effect of the propagation.
