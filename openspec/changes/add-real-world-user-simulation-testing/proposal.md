...
Two contracts this model depends on are **decided here and not yet satisfied by the application**, and are handled by writing the test to the decided contract, quarantining it, and naming the companion change that releases it:

- **CG-1 — day-rollover presentation freshness.** A mounted surface must never label a stale day "Today"; the active section refreshes on the day change and inactive sections refresh on activation. Companion change: `fix-day-rollover-refresh`. (Write correctness across the boundary already holds and is asserted as a passing invariant.)
- **CG-2 — restore emptiness must count deleted rows.** A device that has ever held synced rows is not empty, so a one-shot `INSERT OR REPLACE` import can never resurrect a todo the user deleted while offline. Companion change: `fix-restore-emptiness-counts-deleted-rows`.

Deliberately **not** changed: existing unit tests, existing E2E specs, application source. This change adds a testing layer and its harness; it does not fix any application defect it discovers — including the two above. Defects found while implementing are filed, not silently patched, and no assertion is ever weakened to match current behaviour.
...
- **New dev dependency**: `better-sqlite3` for the integration level, chosen over `node:sqlite` so this change does not also require moving CI from Node 20 to 22+. The driver sits behind a five-method adapter, so that switch is a one-file change whenever the Node version moves for its own reasons. No runtime dependency changes; the app bundle is untouched.
- **Runtime/CI cost**: journeys are long by design. Expected +6–10 minutes of E2E wall-clock for the CI regression subset (`workers: 1` locally is still mandatory — OPFS holds one SQLite lock per origin). The full journey set, including multi-day and volume journeys, runs on `main` and nightly, not on every PR. Sync journeys additionally need a second export (`dist-sync/`, built with obviously-fake Supabase env values), which is confined to the `main`/nightly lane so pull-request feedback stays fast.
...
- **Follow-up changes this proposal creates**: `fix-day-rollover-refresh` (CG-1), `fix-restore-emptiness-counts-deleted-rows` (CG-2), and a product note recommending Pomodoro resume-after-reload (persisting `startedAt` + mode + duration to `app_meta`). The first two release quarantined tests when they land; the third is a UX improvement no test depends on.
