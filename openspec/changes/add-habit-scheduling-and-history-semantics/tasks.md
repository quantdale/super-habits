...
- [ ] 6.5 Apply the repository-managed additive migration to the intended Supabase development/disposable target and prove a Habit V2 remote upsert/restore round trip. **Blocked: this workstation has no authorized target/project reference, Supabase access token, public client configuration, linked project, Docker-backed local database, or remote round-trip environment.**
