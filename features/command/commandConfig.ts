...
export function getAiCommandParseConfig(): AiCommandParseConfig {
  ...
  return {
    ...
    internalRolloutEnabled: readBooleanFlag(process.env.EXPO_PUBLIC_AI_COMMAND_INTERNAL_ROLLOUT),
...
export function isAiCommandRemoteBackendConfigured(config = getAiCommandParseConfig()): boolean {
...
export function isAiCommandInternalRolloutAvailable(config = getAiCommandParseConfig()): boolean {
  return (
