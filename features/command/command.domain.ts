...
function cleanEntityLabel(text: string): string | null {
  const cleaned = stripLeadingScaffolding(text.replace(/[.,!?;:]+$/g, ''), [
    /^to\s+/i,
    /^that\s+/i,
  ]);
...
function detectIntent(rawText: string): {
  ...
  if (hasINeedTo) {
    const remainder = stripLeadingScaffolding(leadingNormalizedText, [I_NEED_TO_PREFIX_PATTERN]);
...
function extractHabitTargetPerDay(text: string): {
  ...
  const patterns: { pattern: RegExp; value: number }[] = [
...
export function parseCommandDraft(input: ParseCommandInput): ParseCommandResult {
  ...
  if (!intent) {
    return {
      outcome: 'unsupported',
      rawText,
      reason: reason ?? 'Use todo or habit wording so the command shell knows what to create.',
    };
  }
