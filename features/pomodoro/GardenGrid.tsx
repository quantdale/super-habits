...
function MiniPlant({ color = SECTION_COLORS.focus }: { color?: string }) {
  return (
    <Svg width={32} height={32} viewBox="0 0 32 32">
      ...
      <Line x1={16} y1={26} x2={16} y2={10} stroke={color} strokeWidth={2.5} strokeLinecap="round" />
...
export function GardenGrid({ sessions, accentColor = SECTION_COLORS.focus }: Props) {
  ...
  if (sessions.length === 0) {
    return (
      <View
        className="mx-1 my-2 items-center rounded-xl border p-4"
        style={{ borderColor: colors.border, backgroundColor: colors.cardSecondary }}
      >
  ...
  return (
    <View>
      <Text className="mb-3 px-1 text-xs" style={{ color: colors.textSubtle }}>
        Your garden — {sessions.length} session
        {sessions.length !== 1 ? "s" : ""}
      </Text>
      <View className="flex-row flex-wrap gap-3 px-1">
        {sessions.map((session) => (
          <Pressable key={session.id} className="items-center" onPress={() => undefined}>
            <MiniPlant color={accentColor} />
            <Text className="mt-0.5 text-xs" style={{ color: colors.textSubtle }}>
