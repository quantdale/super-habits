...
test.describe('Command evaluation (mock/default path)', () => {
  ...
  test('captures parser matrix quality and semantic draft quality in mock mode', async ({
    ...
    if (artifact.unavailableCount !== 0) {
      summaryIssues.push(
        `Unexpected unavailable outcomes observed in mock mode: ${artifact.unavailableCount}`,
      );
    }
    if (artifact.metadataVisibleCount !== 0) {
      summaryIssues.push(
        `Internal metadata was visible in mock/public mode for ${artifact.metadataVisibleCount} cases.`,
      );
    }
