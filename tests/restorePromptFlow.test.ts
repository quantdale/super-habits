...
function buildEntityStatuses() {
  const entities: SyncBackedEntity[] = ['todos', 'habits', 'calorie_entries', 'workout_routines'];
...
describe('restorePromptFlow', () => {
  it('keeps the prompt visible and uses direct blocked message from restore result', () => {
    ...
    const refreshedPreview = buildBlockedPreview('Fallback message from refreshed preview.');
  ...
  it('falls back to refreshed preview message when direct blocked reason is empty', () => {
    const resultPreview = buildBlockedPreview('   ');
    const refreshedPreview = buildBlockedPreview('Remote backup is disabled in local-only mode.');
    ...
    expect(outcome.dismissPrompt).toBe(false);
    expect(outcome.errorMessage).toBe('Remote backup is disabled in local-only mode.');
  });
