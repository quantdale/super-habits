...
async function loadDbClient(options: LoadClientOptions = {}) {
  ...
  const schemaVersion = options.schemaVersion === undefined ? '13' : options.schemaVersion;
...
describe('core/db/client', () => {
  ...
  it('applies migrations from version 0 and records the v15 owner-binding write (final version is 24)', async () => {
  ...
  it('adds notification action idempotency storage in migration 13', async () => {
    const { client, db } = await loadDbClient({ schemaVersion: '12' });
    ...
    expect(
      sqlCalls.some((sql) =>
        sql.includes('CREATE TABLE IF NOT EXISTS processed_notification_actions'),
      ),
    ).toBe(true);
    expect(
      sqlCalls.some((sql) => sql.includes('idx_processed_notification_actions_processed_at')),
    ).toBe(true);
    expect(
      db.runAsync.mock.calls.some(
        ([sql, args]) =>
  ...
  it('adds the durable outbox owner binding in migration 15', async () => {
    const { client, db } = await loadDbClient({ schemaVersion: '14' });

    await client.getDatabase();

    expect(
      db.runAsync.mock.calls.some(([sql]) =>
        String(sql).includes('ALTER TABLE sync_outbox ADD COLUMN owner_user_id TEXT'),
      ),
    ).toBe(true);
    expect(
      db.runAsync.mock.calls.some(
  ...
  it('adds deep Gym V2 exercise semantics in migration 23', async () => {
    const { client, db } = await loadDbClient({ schemaVersion: '22' });

    await client.getDatabase();

    const alterCalls = db.runAsync.mock.calls.map(([sql]) => String(sql));
    expect(alterCalls.some((sql) => sql.includes('ALTER TABLE routine_exercises'))).toBe(true);
    expect(alterCalls.some((sql) => sql.includes('ALTER TABLE workout_session_exercises'))).toBe(
      true,
    );
    expect(alterCalls.some((sql) => sql.includes('ALTER TABLE custom_exercises'))).toBe(true);
    expect(
      db.runAsync.mock.calls.some(
        ([sql, args]) =>
          String(sql).includes('INSERT OR REPLACE INTO app_meta') &&
  ...
  it('adds history-table range indexes in migration 24', async () => {
    const { client, db } = await loadDbClient({ schemaVersion: '23' });

    await client.getDatabase();

    const sqlCalls = db.execAsync.mock.calls.map(([sql]) => String(sql));
    expect(sqlCalls.some((sql) => sql.includes('idx_pomodoro_sessions_started_at'))).toBe(true);
    expect(sqlCalls.some((sql) => sql.includes('idx_workout_logs_completed_at'))).toBe(true);
    expect(sqlCalls.some((sql) => sql.includes('idx_habit_completions_date_key'))).toBe(true);
    expect(sqlCalls.some((sql) => sql.includes('idx_todos_pending_sort'))).toBe(true);
    expect(
      db.runAsync.mock.calls.some(
        ([sql, args]) =>
          String(sql).includes('INSERT OR REPLACE INTO app_meta') &&
  ...
  it('closes the opened handle when bootstrap fails so retries cannot stack connections', async () => {
    vi.resetModules();
    const db = buildDb('8');
    db.runAsync.mockImplementation(async (sql: string) => {
      if (String(sql).includes('ADD COLUMN recurrence')) {
  ...
  it('treats an unparsable stored schema version as 0 instead of silently skipping every migration', async () => {
    const { client, db } = await loadDbClient({ schemaVersion: 'garbage' });

    await client.getDatabase();

    // Every block re-ran from the safe default, ending with the current head.
    const versionBump = db.runAsync.mock.calls.find(
      ([sql, args]) =>
        String(sql).includes('INSERT OR REPLACE INTO app_meta') &&
        Array.isArray(args) &&
