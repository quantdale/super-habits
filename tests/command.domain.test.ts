...
describe('features/command/command.domain (v1 rules hardened)', () => {
  ...
  it.each(['Create a habit', 'Create a habit to'])(
    'treats scaffold-only habit command %p as needs_input',
    (rawText) => {
      const result = parseCommandDraft({
        ...PARSE_INPUT_BASE,
        rawText,
  ...
  it.each(['Add a todo', 'Add a task', 'Create a task', 'Remind me to'])(
    'treats scaffold-only todo command %p as needs_input',
    (rawText) => {
      const result = parseCommandDraft({
        ...PARSE_INPUT_BASE,
        rawText,
  ...
  it('parses habit target phrase variants', () => {
    const cases: { rawText: string; expectedTarget: number }[] = [
