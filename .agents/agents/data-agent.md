...
- Every recoverable write calls syncEngine.enqueue() through the durable mutation helpers; only linked-action events/executions and processed notification actions are local operational state.
...
- Current schema version: **24** — next migration: new **`if (version < 25)`** block in `runMigrations()` in `core/db/client.ts` (when a schema change is introduced)
