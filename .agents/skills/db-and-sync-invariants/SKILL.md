...
- Hard delete is the allowed exception for this table; `habit_completions` rows are still a synced backup entity, so the corresponding durable outbox delete intent is written through the normal mutation path.
