...
| **`core/`**      | Cross-cutting infra: **DB singleton + migrations** (`core/db/client.ts`), **entity types** (`core/db/types.ts`), **app_meta key registry** (`core/db/appMeta.ts`), **account ownership/coordinator** (`core/auth/*`), **sync queue** (`core/sync/sync.engine.ts`), **restore v1 coordinator + types** (`core/sync/restore.*`), **Linked Actions** (`core/linked-actions/*`), **in-app notices** (`core/notifications/` — `inAppNotices.store.ts` + `inAppNotices.types.ts`, surfaced by `core/providers/InAppNoticeProvider.tsx`), **theme tokens + 14-theme registry** (`core/theme/` — `registry.ts`, `tokens.ts`, `themes/`), provider bootstrap (`core/providers/AppProviders.tsx`), theme state (`core/providers/ThemeProvider.tsx`), section-switch state (`core/providers/NavigationProvider.tsx`), PWA SW registration (`core/pwa/registerServiceWorker.ts`), shared **`core/ui/`** primitives. |
