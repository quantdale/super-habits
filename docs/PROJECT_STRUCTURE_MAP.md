...
| **`features/`**  | Product modules: `{feature}.data.ts` (SQLite + enqueue), optional `{feature}.domain.ts` (pure), `*Screen.tsx` + subcomponents, `types.ts` barrel, `features/shared/` for cross-feature UI + shared types (`GitHubHeatmap.tsx`, `activityTypes.ts`). Screen-only/view-variant exceptions are `features/overview/`, `features/settings/` (plus the restore-preview helper `settingsRestorePreview.ts`), and the planning modules (`daily-plan/`, `goals/`, `projects/`, `progress/`, `activity/`, `momentum/`, `planning-hub/`, `weekly-review/`) plus `quick-capture/`; `features/command/` is an overlay-first shell with `CommandCenterProvider.tsx`, parser/config helpers, and an executor instead of a normal feature data file.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| **`tests/`**     | Vitest: unit project (`tests/**/*.test.ts` + `core/**/__tests__/**/*.test.ts`) and integration project (`tests/integration/**/*.test.ts` against real better-sqlite3 via `tests/integration/helpers/db.ts`); domain/data contracts, command parser/config/executor, linked actions, restore/settings flows, sync engine, and selected DB/provider tests.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |

**Also:** `e2e/` Playwright (+ `playwright.config.ts`, `scripts/serve-e2e.js`, feature + journey spec files + helpers; verify the inventory with `npx playwright test --list`); `public/` static (`sw.js`, `manifest.json`); `assets/` images; `patches/` patch-package; deployment config `vercel.json` (web PWA) and `eas.json` (native builds); Expo app config in `app.json`.
...
| Concern         | File                                                                                            | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Persistence** | `core/db/client.ts`                                                                             | `getDatabase()`, `initializeDatabase()`, bootstrap DDL, **append-only** `runMigrations()` (`if (version < N)` blocks; current v25). WAL native-only. `schema.sql` = hand-maintained reference snapshot **not** runtime; it lags the runtime DDL (the runtime authority is `runMigrations()`); keep it aligned when touching tables it documents. `core/db/migrations/` holds remote/Supabase reference SQL only — local migrations never live there. |
...
| `sset`                      | `workout_session_sets`                                         |
| `wplan`                     | `workout_weekly_plan`                                          |
| `wover`                     | `workout_schedule_overrides`                                   |
| `bw`                        | `body_weight_entries`                                          |
| `cex`                       | `custom_exercises`                                             |
| `pom`                       | `pomodoro_sessions`                                            |
| `ppre`                      | custom pomodoro presets (`app_meta` payload)                   |
...
| `dplan`                     | `daily_plans`                                                  |
| `wrev`                      | `weekly_reviews`                                               |
| `link`                      | `linked_action_rules`                                          |
| `lchain` / `levt` / `lexec` | linked-action chains / events / executions (local operational) |
| `cmd`                       | command history/drafts (`app_meta` local)                      |
