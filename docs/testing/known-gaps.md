...
**2026-09-23 campaign note for gap 15 (Windows, corrected `dist/`):** the full `npm run qa:full` battery at documentation-only descendant `5cd573a` recorded 681 ms (14.9% headroom; 1 ms short of the 15% floor, while under the 800 ms ceiling). The corrected-tree full `e2e:journeys` battery later recorded 702 ms (12.3% headroom, still under the ceiling); the corrected full `qa:full` browser battery then recorded 793 ms (0.9% headroom). Immediate standalone comparisons on that freshly rebuilt `dist/` measured 737 ms (7.9% headroom; the persona stopped after the threshold assertion) and, earlier on the same corrected code, 665 ms (16.9%, full persona 7/7). No section-switch or section-render code changed; the product fix serialized account-owner binding transactions. The 72 ms spread between standalone results and loaded-host full-battery results matches the registered host-variance pattern, though the 793 ms result is only 7 ms below the hard ceiling. Keep both the 800 ms ceiling and 15% floor unchanged.

**2026-09-23 follow-up (same corrected source, host under sustained CPU load):** another full-battery P2 HEAVY overview→Todos switch measured 861 ms; an isolated replay measured 1006 ms while host CPU averaged 81.2% immediately before and 85.1% after. The replay screenshot shows the Todos screen rendered; this is classified `ENVIRONMENT` under host load. The deterministic long-history suite also timed out switching to Calories: the section root mounted, then reached full opacity at 5,128 ms just after the 5 s oracle timeout; post-run host CPU averaged 71.5%. A later CPU sample following the full sync lane averaged 74% (range 71.1–80.7%). These results support the registered host-load sensitivity; no app timing, oracle timeout, 800 ms ceiling, or 15% floor was changed. Re-run the timing checks when host CPU is below 60%.

**2026-09-23 CI-runner evidence for gap 15 (clean GitHub-hosted runners, unchanged guard):** push run `35769774260` failed the floor with 743/790/721 ms; push run `35749583938` (`cc6889a`) passed; exact-SHA run `35809827092` (`e9c65b6`) failed with 766/692/710 ms across its three attempts — 800 ms ceiling held in every CI attempt, 15% floor (≤680 ms) achieved on some runs and missed on others with identical product code. The earlier closing-path assumption that a quiet CI runner would reliably clear the floor is therefore not proven: the measured band straddles 680 ms on CI hardware too, consistent with the CDP attribution (harness ~31% + browser ~53% + app ~16%, no app hotspot). Guard, ceiling, and floor remain unchanged; each CI floor miss is recorded here with its numbers.
...
### 19. Settings overlay text contrast — CLOSED (2026-09-14)

**Reason:** the 2026-09-14 DOM-level WCAG audit (`e2e/a11y.spec.ts`) was clean
for the six product sections but still found a bounded set inside the Settings
...
`#F59E0B` 1.73-1.96:1). The shared surfaces were fixed in the same campaign
(`SettingsStatusPill`, `SettingsSectionHeading`, `Card` header bands,
`PillChip`, `SegmentedControl`, `StatBlock`, `Button`, `EmptyStateCard`), so
what remained was the residual set the fix's surface approximation could not
fully reach (nested tints).

**Resolution:** `SettingsStatusPill` now resolves its accent text against a
18% tint at a 7:1 target so a nested tint cannot defeat it;
`SettingsNotificationsSection` derives one `accentText` for every accent label
(Save buttons, active weekday chips, effort-scale chips) and keeps the raw
hue for tints and borders; `ThemePreviewCard`'s theme name uses `textMuted`
rather than the icon-only `iconMuted`. The audit's Settings contrast case now
runs unskipped in `e2e/a11y.spec.ts` (3 passed, no `test.fixme`).

**Closing path:** closed. Keep the Settings contrast assertion enabled; a new
