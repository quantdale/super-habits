...
## Schema version
- Current stored version: **23** (`app_meta.db_schema_version`)
- Next migration number: **24** (add `if (version < 24) { ... }` in `runMigrations()` in `core/db/client.ts` when a schema change lands)
