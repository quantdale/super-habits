...
- Current stored schema version: 25. New migration: append a new `if (version < 26) { ... }` block at the end of `runMigrations()` in `core/db/client.ts` (there are no numbered migration files).
