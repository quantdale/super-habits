...
- Current stored schema version: 24. New migration: append a new `if (version < 25) { ... }` block at the end of `runMigrations()` in `core/db/client.ts` (there are no numbered migration files).
