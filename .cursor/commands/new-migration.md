...
- Current stored schema version: 23. New migration: append a new `if (version < 24) { ... }` block at the end of `runMigrations()` in `core/db/client.ts` (there are no numbered migration files).
