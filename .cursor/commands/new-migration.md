...
- Current stored schema version: 12. New migration: append a new `if (version < 13) { ... }` block at the end of `runMigrations()` in `core/db/client.ts` (there are no numbered migration files).
