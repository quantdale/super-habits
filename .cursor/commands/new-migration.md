...
- Current stored schema version: 21. New migration: append a new `if (version < 22) { ... }` block at the end of `runMigrations()` in `core/db/client.ts` (there are no numbered migration files).
