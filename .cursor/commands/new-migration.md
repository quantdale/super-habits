...
- Current stored schema version: 22. New migration: append a new `if (version < 23) { ... }` block at the end of `runMigrations()` in `core/db/client.ts` (there are no numbered migration files).
