...
- Current schema version: **8** — next migration: new **`if (version < 9)`** block in `runMigrations()` in `core/db/client.ts` (when a schema change is introduced)
...
- 68 tests must pass after every change — update this count whenever tests are added or removed
