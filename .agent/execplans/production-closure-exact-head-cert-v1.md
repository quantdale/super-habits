...
- Current milestone: **Phase 3 COMPLETE at application candidate `cc6889af2eba707cfbbf17792888f63a74b489af`; Phase 4 Android exact-build certification is in progress.** The canonical application APK was built and installed from documentation-only descendant `19191bd83ef8bb12db41e2a565abe1debb898beb` on `Nitro_API_36` (API 36, x86_64); SHA-256 `2DEB2276C3CA8D9707A49518A911188CD8804CFB5FB413B3CA154075EA676716`. CI run `35769774260` is terminal: `quality` SUCCESS, `e2e` FAILURE on the existing J8 section-switch headroom guard (743/790/721 ms against the unchanged 800 ms ceiling and 15% headroom floor), nightly skipped by design. Earlier candidate run `35749583938` had quality and e2e SUCCESS.
...
- In progress: Phase 4. Repository-owned provisioning built/installed the current canonical APK; Android smoke passed 2/2 and targeted persistence passed 11/11. Lifecycle failed 2/6 with four failures localized to flows that do not reveal newly created content or tap a bottom-edge button safely. Same-APK `habit-persistence` control passed end-to-end, including create, scroll-to-item, complete, and relaunch. Current evidence classifies those four as `TEST_BUG`; flow corrections are in place in `.maestro/flows/habit-reminder-actions.yaml`, `.maestro/flows/habit-reminder-actions-replay.yaml`, `.maestro/flows/habit-reminder-delivery.yaml`, and `.maestro/flows/workout-gym-v2-session-lifecycle.yaml`, pending exact-head native replay. Because the tracked tree must remain clean and source provenance exact, these fixes and the current checkpoint will be committed together before rebuilding and replaying Android lanes.
- Important modified files: The four lifecycle flow files above plus this ExecPlan are in progress. No application source changes are indicated by current evidence.
- Last successful validation: 2026-09-23 — current APK provision PASS (source `19191bd`, hash above); Android smoke 2/2 PASS; targeted persistence 11/11 PASS; isolated `habit-persistence` flow PASS on that same APK; Phase 3 exact-candidate CI run `35749583938` quality/e2e SUCCESS (2247P/1S, 23 deterministic scenarios, sync 40P/6S); local `sim:validate`, `qa:fast`, focused ExecPlan tests, and plan validation PASS.
- Current failures: Preserved lifecycle report `simulation-output/native/native-android-lifecycle-Nitro_API_36-2026-09-22T193907546Z.json` is `FAILED_NEEDS_TRIAGE`, 2/6 flows passed / 4 failed. Artifacts show habit records were not in the visible hierarchy at assertion time; the passing control scrolls UP and centers the newly created item. Workout lifecycle tapped `Add routine` at y=2246 (under the floating section bar/taskbar); the failure screenshot shows To Do selected. The background heartbeat-file lock messages did not interrupt execution and are not causal based on current evidence. CI run `35769774260` additionally failed the J8 headroom guard at 743/790/721 ms; thresholds remain unchanged and the exact-candidate CI run had passed. This perf failure remains under investigation for the mandatory Phase 9 adversarial audit.
...
- Exact next action: stage only the four corrected Maestro lifecycle flows and this ExecPlan, commit the validated checkpoint locally, then force-build that exact SHA and rerun Android smoke, lifecycle, and targeted persistence before `qa:full`.
