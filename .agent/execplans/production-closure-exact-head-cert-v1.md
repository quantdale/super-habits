...
- In progress: Phase 4. Repository-owned provisioning built/installed the prior canonical APK; Android smoke passed 2/2 and targeted persistence passed 11/11. Lifecycle failed 2/6 with four failures localized to flows that do not reveal newly created content or tap a bottom-edge button safely. Same-APK `habit-persistence` control passed end-to-end, including create, scroll-to-item, complete, and relaunch. Current evidence classifies those four as `TEST_BUG`; flow corrections are committed in `9ca8200` in `.maestro/flows/habit-reminder-actions.yaml`, `.maestro/flows/habit-reminder-actions-replay.yaml`, `.maestro/flows/habit-reminder-delivery.yaml`, and `.maestro/flows/workout-gym-v2-session-lifecycle.yaml`, pending exact-head native replay. A plan-only checkpoint will be committed next, then a force build will establish the new exact provenance SHA for Android reruns.
- Important modified files: Only this ExecPlan is being updated for the committed flow checkpoint. No application source changes are indicated by current evidence.
...
- Exact next action: record the flow-fix commit in this validated checkpoint, commit the plan update, then run `npm run qa:native:provision -- --force --serial emulator-5554` with Node 22.23.2 and certify the resulting exact SHA.
