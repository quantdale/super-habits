...
- Last successful validation: `npm run qa:fast` PASS (typecheck, lint, 613 unit tests); full `npm test` PASS (664/664); timezone matrix PASS; deterministic P0 simulation PASS; static Chromium/journeys/simulation matrix 146 passed with only the documented D14 environment miss; dummy-env sync build PASS and explicit `dist-sync` sync project 19/19 passed.
...
- 2026-08-12 — `E2E_PORT=8228 npx playwright test --project=journeys-sync` without `E2E_DIST_DIR` — PASS / STANDARD EXPORT; 9 passed and 10 runtime-gated steps skipped. This invocation served the standard export and is superseded for sync-boundary evidence by the explicit `dist-sync` run below.
- 2026-08-12 — `E2E_PORT=8229 E2E_DIST_DIR=dist-sync npx playwright test --project=journeys-sync` — PASS; all 19 dummy-boundary tests passed in 2.0 minutes, including reconnect and restore paths, with no remote mutation.
