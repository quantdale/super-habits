...
- Finding W8-4 (P1 SUSPECT, product-read vs harness undetermined — Wave 10 must resolve before campaign close): calories-persistence post-restart diary shows EMPTY (Today 0 kcal, 'Log food to start tracking', screenshot proof, 3 consecutive runs) while a root-pulled device DB dump proves the row EXISTS (`Native breakfast, 370 kcal, breakfast, consumed_on 2026-09-04, deleted_at NULL`) with outbox create intents (calorie_entries + saved_meals) and schema 24. NO DATA LOSS (P0 dead). Pre-restart save verified genuine ('Logged today' header renders only with entries.length>0; prior run's 370-kcal diary card). Save path is transactional INSERT via runSyncedMutation; diary refresh fires on isActive (useActiveForegroundRefresh) so stale-mount theory is weak. First Wave-10 probe: post-restart Form view (distinguishes read-layer vs diary-layer) + instrumented repro. Product code UNCHANGED on suspicion. Full proof: screenshots + maestro logs + DB query output + replay `node scripts/qa-native.mjs --platform android --flow .maestro/flows/calories-persistence.yaml --avd Nitro_API_36`.
...
- Wave 8H DONE: repeat runner proven under real workloads (unit×5, integration×5, P0×5, sim×1, native-smoke/auth/lifecycle/persistence — all sequential with collated 14-field records under simulation-output/repeat/ + per-target native records). Failure contract verified on real evidence: native-auth-3x collation (3 FAILED_NEEDS_TRIAGE, status FAIL, SHA + replay present, exit 1) + native-smoke-3x BLOCKED collation + config-error path (`--suite bogus` → exit 1 with exact reason). No early abort, no leak (post-kill hygiene verified adb empty/ports free/mock :4545 free). No tooling changes needed.
...
- Wave 8 CLOSED except web:verify #3 (runs after closure commit at final SHA) + Waves 9/10. No product-source changes in Wave 8 (flows/tests/docs only); product tree identical to Wave 7 head except Wave-1 FAB fix.
- Exact next action: Commit Wave 8 closure, push, run web:verify #3 at final SHA, then Wave 9 adversarial verification (verifier must confront W8-1 headroom, W8-3 auth-matching, W8-4 diary-read honestly) + Wave 10 P0/P1 sweep (W8-4 Form-view probe first).
