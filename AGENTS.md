...
- Six tab surfaces (section labels; internal section keys in parentheses): **Today** (`overview`), **To Do** (`todos`), **Habits**, **Focus** (`pomodoro`), **Workout**, and **Calories**.
- A single-page experience: the six sections are rendered inside `app/index.tsx` behind a `NavigationContext.activeSection` state, with a section switcher of plain `Pressable` items — a floating bottom tab bar on narrow viewports and a left side rail on wide viewports. Settings is a full-screen **modal**; the Command Center is a **global overlay** (no `/command` route).
- Local **SQLite** is the source of truth; writes may optionally be pushed to **Supabase** as a backup.
- UI follows the **Pop** design system (`docs/ui-ux/12-pop-design-system.md`): Nunito type through `core/ui/Text`, chunky rounded surfaces, per-section hues via `useAppTheme().sectionAccents`, and tactile motion that honours reduced motion.
- A **local-only reward layer** (migration 25: XP/levels, streaks + freezes, rotating daily quests, 36 badge tiers, celebration overlay, haptics/synth tones) reads the authoritative feature tables and writes only its own four tables — never synced, backed up, or account-owned (`docs/knowledge-base/gamification.md`).
...
| `supabase/`  | Supabase Edge Functions (`supabase/functions/parse-ai-command/`, `supabase/functions/user-ai-ask/`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- `app/index.tsx` renders the six sections (Today, To Do, Habits, Focus, Workout, Calories) behind `NavigationContext.activeSection`, with a section switcher of plain `Pressable` items (floating bottom tab bar / wide-screen side rail).
...
- **Supabase auth** centers anonymous sessions (anonymous sign-in is optional per project config) plus email-OTP account recovery; no user passwords are handled.
