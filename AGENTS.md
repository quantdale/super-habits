...
- Current stored schema version: **22** (`app_meta.db_schema_version`), including durable processed-notification-action state, the SQLite sync outbox and its durable owner binding, the planning entities (16–19), the hardening-wave-v2 durable-state promotion (20: habit lifecycle columns, Pomodoro session metadata columns, `workout_session_sets`, workout timing columns), migration 21 (`daily_plans.top_todo_titles`), and Gym V2's additive routine/session columns plus custom-exercise, weekly-plan, schedule-override, and body-weight tables. Next migration: add a new `if (version < 23) { ... }` block.
