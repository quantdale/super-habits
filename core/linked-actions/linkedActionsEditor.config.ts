...
function buildEditorConfigForSourceFeature(
  feature: LinkedActionFeature,
): LinkedActionsEditorConfig {
  const supportedPathsForSource = LINKED_ACTION_SUPPORTED_RULE_PATHS.filter(
    (path) => path.sourceFeature === feature,
  );

  return {
    allowedTargetFeatures: uniqueValues(supportedPathsForSource.map((path) => path.targetFeature)),
    allowedTriggerTypes: uniqueValues(supportedPathsForSource.map((path) => path.triggerType)),
